...
Дополнительные сущности (например Enum), не используемые в качестве входящих/исходящих для `service`, и не являющиеся полноценным понятием предметной области конечного модуля, но используемые другии сущностями, должны определяться в том файле, в котором находится использующая их сущность.

Недопустимо импортировать файлы `api.proto` в файлы `entities.proto`.
